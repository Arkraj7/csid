...
export default function ClimateAwarenessPage() {
  ...
  return (
    <div className="min-h-screen">
      <Navbar currentPath="/climate-awareness" />
      <main
        className={`${isDarkMode ? 'bg-gradient-to-br from-slate-900 via-indigo-950 to-purple-950' : 'bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50'}`}
      >
        <div className="container mx-auto px-4 py-16">
          {/* Header */}
          <div className="text-center mb-16">
            <h1
              className={`text-5xl font-bold mb-6 font-serif ${isDarkMode ? 'text-slate-100' : 'text-emerald-800'}`}
            >
              Climate Awareness Challenge
            </h1>
            <p
              className={`text-xl max-w-3xl mx-auto leading-relaxed ${isDarkMode ? 'text-indigo-300' : 'text-amber-700'}`}
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {difficulties.map((difficulty) => (
              <div
                key={difficulty.id}
                className={`${isDarkMode ? 'bg-slate-800/60' : 'bg-white/90'} backdrop-blur-sm rounded-2xl p-8 border ${difficulty.borderColor} shadow-lg hover:shadow-xl transition-all duration-300 hover:transform hover:scale-105`}
              >
                {/* Icon */}
                ...
                <div className="text-center">
                  <h3
                    className={`text-2xl font-bold mb-2 ${isDarkMode ? 'text-slate-100' : 'text-emerald-800'}`}
                  >
                    {difficulty.name}
                  </h3>
                  <div
                  ...
                  <p
                    className={`${isDarkMode ? 'text-slate-300' : 'text-gray-600'} text-sm leading-relaxed mb-6`}
                  >
          ...
          <div className="text-center mt-16">
            <Link
              href="/"
              className={`${isDarkMode ? 'text-indigo-300' : 'text-amber-700'} hover:text-emerald-600 dark:hover:text-indigo-400 transition-colors inline-flex items-center gap-2 font-medium`}
            >
              ← Back to Home
            </Link>
          </div>

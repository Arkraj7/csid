...
export const updateUserProgress = async (updates: {
  ...
  try {
    ...
    if (updates.certificatesEarned) updatePayload.certificatesEarned = increment(updates.certificatesEarned);
    if (updates.lessonsCompleted) updatePayload.lessonsCompleted = increment(updates.lessonsCompleted);
